...
def run() :
  ...
  for test in test_list : 
    ...
    if(not args.test or args.test==test.title) : 
      ...
      print("Requests")
      requests_text = build_request_text(test, requests)
      preprocess_requests=tokenize(requests_text, test.pretreatment_type) # print(preprocess_requests)

      # Search results
      results={}
      for index, preprocess_request in enumerate(preprocess_requests,1) :
        # Vectorize requests 
        if("vectors" not in requests_text) : 
          requests_text['vectors'] = vectorizer.vectorize_request(preprocess_request, test.weight_schemat)
        # Search     
        most_similar_docs_indices = search(requests_text['vectors'], documents_vectors) 
      ...
      print("[", test.title.capitalize(), "] ", MAP)
